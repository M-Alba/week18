...
//элементы формы
const gridContainer = document.querySelector('.heroes__gridContainer');
...
document.addEventListener("DOMContentLoaded", function (event) {
    ...
    heroes.forEach(function (item, index) {
        const heroesContent =
            `<section class="heroCard__wrapper">
            <h2>${item.name}</h2>
            <div><b>Вселенная:</b> ${item.universe}</div>
            <div><b>Альтер эго:</b> ${item.alterego}</div>
            <div><b>Род деятельности:</b> ${item.occupation}</div>
            <div><b>Друзья:</b> ${item.friends}</div>
            <div><b>Суперспособности:</b> ${item.superpowers}</div>
            <div>
                <img class="heroCard__image" src="${item.url}" alt="Изображение супергероя">
            </div>
            <div class="heroRating__wrapper">
                <input id="star5-${index}" type="radio" class="heroRating__input" name="rating-${index}" value="5">
                <label for="star5-${index}" class="heroRating__label"></label>
                <input id="star4-${index}" type="radio" class="heroRating__input" name="rating-${index}" value="4">
                <label for="star4-${index}" class="heroRating__label"></label>
                <input id="star3-${index}" type="radio" class="heroRating__input" name="rating-${index}" value="3">
                <label for="star3-${index}" class="heroRating__label"></label>
                <input id="star2-${index}" type="radio" class="heroRating__input" name="rating-${index}" value="2">
                <label for="star2-${index}" class="heroRating__label"></label>
                <input id="star1-${index}" type="radio" class="heroRating__input" name="rating-${index}" value="1">
                <label for="star1-${index}" class="heroRating__label"></label>
            </div>
        </section>`

        //Вывод данных
        gridContainer.insertAdjacentHTML('beforeend', heroesContent);
    });

    const inputs = document.querySelectorAll('.heroRating__input')
    for (const input of inputs) {
        //обработчик события нажатия на input (звездочку)
        input.addEventListener('click', () => {
            //запись рейтинга в локальное хранилище
